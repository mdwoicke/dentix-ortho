/**
 * Update scheduling tool to v33 with SANDBOX_MIN_DATE fix
 */
const fs = require('fs');
const path = require('path');

const docsDir = path.join(__dirname, '..', 'docs', 'v1');

// v33 func code with SANDBOX_MIN_DATE fix
const v33Code = `/**
 * ============================================================================
 * CHORD SCHEDULING DSO - Appointment Scheduling Tool (Node Red Version)
 * Version: v33 | Updated: 2026-01-03
 * ============================================================================
...
const ACTIONS = {
    slots: {
        ...
        buildBody: (params, uui) => ({
            ...
            scheduleViewGUIDs: params.scheduleViewGUIDs || DEFAULT_SCHEDULE_VIEW_GUID
        }),
        validate: () => {},
...
async function executeRequest() {
    ...
    console.log('[' + toolName + '] v33 - 2026-01-03 - SANDBOX_MIN_DATE fix (min Jan 13, 2026)');
...
// Save raw .js file
fs.writeFileSync(path.join(docsDir, 'scheduling_tool_func.js'), v33Code);
console.log('✓ Updated scheduling_tool_func.js');

// Save escaped version for Flowise
const escaped = v33Code.replace(/\{/g, '{{').replace(/\}/g, '}}');
fs.writeFileSync(path.join(docsDir, 'scheduling_tool_func_escaped.js'), escaped);
console.log('✓ Updated scheduling_tool_func_escaped.js');

// Update JSON file
const jsonPath = path.join(docsDir, 'schedule_appointment_dso_Tool.json');
const json = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
json.func = v33Code;
fs.writeFileSync(jsonPath, JSON.stringify(json, null, 2));
console.log('✓ Updated schedule_appointment_dso_Tool.json');

console.log('\n=== All files updated to v33 with SANDBOX_MIN_DATE fix ===');
console.log('Key change: Dates before Jan 13, 2026 are auto-corrected');
