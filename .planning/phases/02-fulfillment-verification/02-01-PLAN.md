...
3. Cache verification results in the session_analysis table by adding columns dynamically in the controller's `getDb()` or database init function (matching the Phase 1 pattern where session_analysis table was auto-created). Do NOT modify schema.sql. Specifically:
   - In the same function that creates/opens the database connection for session_analysis, add ALTER TABLE statements wrapped in try/catch to add: `verification_json TEXT`, `verification_status TEXT`, `verified_at TEXT`
   - Ignore "duplicate column" SQLite errors (error message contains "duplicate column name") so this is safe to run repeatedly
   - If cached verification exists and force=false, return cached result
...
3. Call GET /api/trace-analysis/{sessionId}/verify -- should return verification verdict
  ...
  <done>Existing endpoint supports ?verify=true and returns verification results. Dedicated /verify sub-endpoint exists. Results are cached in session_analysis table via ALTER TABLE in the controller's init logic (not schema.sql). Backward compatible -- omitting verify param returns null verification.</done>
</task>
