...
3. **triggerDiagnostics()** method:
   ...
   - For each: dynamically import diagnosticOrchestrator using explicit dynamic import:
     ```typescript
     const { DiagnosticOrchestrator } = await import('./diagnosticOrchestrator');
     ```
   - Call the orchestrator's analyze() method on the session
...
- Dynamic import() for diagnosticOrchestrator to avoid circular deps (see snippet above)
...
- Log each session analysis result to console for observability
  ...
  <done>
    MonitoringService exists with runMonitoringCycle() and triggerDiagnostics() methods. triggerDiagnostics uses `await import('./diagnosticOrchestrator')` for dynamic loading. monitoring_results table auto-created. Singleton exported.
  </done>
</task>
...
2. **In traceAnalysisController.ts**, export `getDb` function (or make it accessible) so MonitoringService can reuse the same DB connection pattern. If getDb is already module-scoped, add `export` keyword. Also ensure `transformToConversationTurns` and `filterInternalTraces` are importable (they should already be exported from testMonitorController).
  ...
  <verify>
    `cd backend && npx tsc --noEmit` compiles without errors. Verify wiring with: `grep -A5 "runMonitoringCycle" backend/src/services/heartbeatService.ts` confirms the call exists after evaluateAlerts.
  </verify>
  <done>
    HeartbeatService runs monitoring cycle every 5 minutes (gated by timestamp check within 1-min heartbeat). Failed sessions auto-trigger diagnostics. Monitoring errors are caught and logged without breaking alert evaluation. `grep runMonitoringCycle heartbeatService.ts` shows the wiring.
  </done>
</task>
...
3. `grep -A5 "runMonitoringCycle" backend/src/services/heartbeatService.ts` confirms wiring after evaluateAlerts
4. MonitoringService.triggerDiagnostics() uses `await import('./diagnosticOrchestrator')` pattern
5. MonitoringService.triggerDiagnostics() caps at 3 sessions per invocation
</verification>
