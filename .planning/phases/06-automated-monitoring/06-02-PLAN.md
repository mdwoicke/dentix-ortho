...
<task type="auto">
  <name>Task 2: Add API client, filter bar, and monitoring results list to CallTracePage</name>
  <files>frontend/src/services/api/testMonitorApi.ts, frontend/src/pages/TestMonitor/CallTracePage.tsx</files>
  <action>
**Scope note:** This task intentionally combines the API client, filter bar, and results list into a single task because they are tightly coupled (filters drive the API call which populates the list). The UI is a single collapsible section added to an existing page, not a standalone page build. Total is ~150-200 lines of new JSX which fits within a single task.
...
2. **In CallTracePage.tsx**, add a collapsible "Monitoring Filters" section above or alongside the existing session list:

   **Filter bar** (single row, compact):
   - Date range: two `type="date"` inputs (dateFrom, dateTo)
   - Status: toggle buttons (All, Pass, Fail, Partial, Skipped) -- multi-select style
   - Intent type: `<select>` dropdown (booking, cancellation, rescheduling, info_lookup, other)
   - Session ID: text input with 300ms debounce

   **Results list** (below filter bar):
   - Table with columns: Session ID (truncated, clickable), Intent Type, Status badge, Confidence, Analyzed At
   - Status badges use Tailwind: `bg-green-100 text-green-800` (pass), `bg-red-100 text-red-800` (fail), `bg-yellow-100 text-yellow-800` (partial), `bg-gray-100 text-gray-800` (skipped/error)
   - Clicking session ID sets search params to load that session in existing trace viewer
   - Total count display + Previous/Next pagination buttons
   - Default: latest 50 results, no filters

   Style consistently with existing Tailwind patterns (Card components, text-sm).
  </action>
  <verify>
