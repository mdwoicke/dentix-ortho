...
3. In the `analyze()` method, after parsing the LLM response, if `suggestedCode` is non-null and non-empty:
   ...
   - Detect whether `suggestedCode` is a full file replacement or a partial snippet. Heuristic: if `suggestedCode.length < currentArtifact.length * 0.5` OR it doesn't contain typical file-level markers (e.g., no `import` or `require` at the top for JS/TS files, no `#` header for markdown), treat it as a partial snippet.
   - **Full replacement:** Generate unified diff: `createTwoFilesPatch(fileKey, fileKey, currentArtifact, suggestedCode, 'current', 'proposed')`. Set `result.unifiedDiff = diff`.
   - **Partial snippet:** Do NOT attempt a full-file diff (it would be meaningless). Instead, set `result.unifiedDiff` to a formatted block: `"// PARTIAL REPLACEMENT — apply manually to the relevant section\n// Suggested code:\n" + suggestedCode`. Also add `result.isPartialDiff = true` (add this boolean field to `ExpertAnalysisResult`).
