...
2. Modify `diagnoseProductionTrace` (around line 8904): Replace the generic LLM prompt section. After the existing trace loading and transcript building code (keep lines 8811-8866 as-is), replace the LLM call section (lines ~8888-9048) with:
   ...
   - Build `DiagnosticRequest` from the existing transcript, apiErrors, and add stepStatuses by running `mapToolSequence()` on the observations (import from toolSequenceMapper if not already imported; use the existing `callerIntentClassifier` result if available from session_analysis cache, or create a minimal intent for the mapper)
   ...
   - Keep backward compatibility: response still has `fixesGenerated`, `summary`, etc. but adds `diagnosticReport` field with the orchestrator's combinedMarkdown
