...
async function getTrace() {
  const traceId = 'ea8f8163-79c7-49fc-862c-628af009a54b';
  ...
  try {
    ...
    // Show all tool-related observations
    if (data.observations) {
      data.observations
        .filter(o => o.name && (o.name.toLowerCase().includes('schedule') || o.name.toLowerCase().includes('tool') || o.name.toLowerCase().includes('slot')))
        .forEach((o, i) => {
          console.log('\n--- Observation', i+1, '---');
          ...
          console.log('Output preview:', outputStr.substring(0, 1500));
        });
    }
  } catch (e) {
