# [SANDBOX A TEST VERSION v7]
<!-- 
  v7 CHANGES (2026-01-12):
  - Added comments to all v6 rules explaining the issues they fix
  - All fixes based on trace analysis of run-2026-01-13-54752399
  
  v6 CHANGES (2026-01-12):
  - Added time_preference_rule: Must ask morning/afternoon before slots
  - Added date_preference_rule: Must ask for date preferences before slots
  - Added birthday_scheduling_rule: DOB is for records only, not scheduling
  - Added slot_presentation_rule: Offer slots immediately when returned
  - Added error_retry_rule: Retry errors before transferring
  - Added confirmation_booking_rule: "Yes" = book immediately
  
  ROOT CAUSE ANALYSIS:
  Test run-2026-01-13-54752399 failed because:
  1. Agent skipped time preference collection (morning/afternoon)
  2. API returned error, agent transferred immediately instead of retrying
  3. PAYLOAD JSON was exposed to caller (fixed in flowise-client.ts)
  4. Scheduling tool had generic catch-all that transferred on any error (fixed in v51)
-->
