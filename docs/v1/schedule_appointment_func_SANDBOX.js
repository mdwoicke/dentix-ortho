 ...
 * Version: v56 | Updated: 2026-01-14
 * ============================================================================
 * Actions: slots, grouped_slots, book_child, cancel
 *
 * v56 FIX: 30-DAY SLIDING WINDOWS - Cloud9 accepts 30+ day ranges
 *          - Changed from 14-day to 30-day windows for faster slot finding
 *          - Only 2 windows needed (60 days total coverage)
 *          - Slides forward in time instead of expanding from start
 * v55 FIX: RATE LIMIT DELAY - 3s delay between sliding window API calls
 * v54 FIX: SLIDING WINDOW SEARCH - Cloud9 API limit workaround
...
// v56: Sliding window config - 30-day windows for faster slot finding
const SLIDING_WINDOW_CONFIG = {
    windowDays: 30,        // 30-day windows (Cloud9 accepts this)
    maxWindows: 2,         // Only 2 windows needed (60 days total)
    delayBetweenWindowsMs: 3000 // 3s delay to avoid rate limiting
};
...
// v56: Get the start date for sliding window search
function getSearchStartDate(startDate) {
    ...
    if (correctedStart && correctedStart > maxFutureDate) {
        console.log('[v56] WARNING: startDate ' + startDate + ' is too far in future - AUTO-CORRECTING to today');
        correctedStart = null;
    }
    ...
    return formatDate(correctedStart);
}

// v56: Calculate end date for a sliding window
function getWindowEndDate(startDateStr, windowDays) {
...
// v56: Delay helper to avoid Cloud9 rate limiting
function delay(ms) {
...
// v56: Dynamic slot search with SLIDING WINDOWS (30-day windows)
async function searchSlotsWithSlidingWindow(action, params, uui, headers) {
    const config = ACTIONS[action];
    let lastError = null;
    const { windowDays, maxWindows, delayBetweenWindowsMs } = SLIDING_WINDOW_CONFIG;
    const totalDaysSearched = windowDays * maxWindows;

    // v56: Get corrected start date
    let currentStartDate = getSearchStartDate(params.startDate);

    for (let windowIndex = 0; windowIndex < maxWindows; windowIndex++) {
        // Add delay before subsequent windows to avoid rate limiting
        if (windowIndex > 0 && delayBetweenWindowsMs > 0) {
            console.log('[v56] Waiting ' + (delayBetweenWindowsMs / 1000) + 's before next window...');
            await delay(delayBetweenWindowsMs);
        }

        const currentEndDate = getWindowEndDate(currentStartDate, windowDays);

        const searchParams = { ...params, startDate: currentStartDate, endDate: currentEndDate };
        const body = config.buildBody(searchParams, uui);

        console.log('[v56] Window ' + (windowIndex + 1) + '/' + maxWindows + ': ' + currentStartDate + ' to ' + currentEndDate + ' (' + windowDays + ' days)');
        ...
        try {
            ...
            if (hasResults) {
                data._slidingWindow = true;
                data._windowIndex = windowIndex;
                data._totalWindows = maxWindows;
                data._dateRange = { start: currentStartDate, end: currentEndDate, windowDays: windowDays };
                if (windowIndex > 0) {
                    console.log('[v56] Found slots in window ' + (windowIndex + 1) + ' (' + currentStartDate + ' to ' + currentEndDate + ')');
                }
                return { success: true, data: data };
            }

            // No results in this window, slide to next
            console.log('[v56] No slots in window ' + (windowIndex + 1) + ', sliding to next...');
            currentStartDate = currentEndDate;

        } catch (e) {
            lastError = e.message;
            console.log('[v56] Search error in window ' + (windowIndex + 1) + ': ' + e.message);
            currentStartDate = currentEndDate;
        }
    }

    // All windows exhausted, no slots found
    console.log('[v56] All ' + maxWindows + ' sliding windows exhausted, no slots found in ' + totalDaysSearched + ' days');
    return {
        success: false,
        data: {
            slots: [],
            ...
            _slidingWindow: true,
            _windowsSearched: maxWindows,
            _totalDaysSearched: totalDaysSearched,
            _debug_error: lastError || 'No slots available after searching ' + totalDaysSearched + ' days with sliding windows',
            llm_guidance: {
                error_type: 'no_slots_after_sliding_window',
                voice_response: 'I apologize, but I was not able to find any available appointments within the next ' + Math.round(totalDaysSearched / 7) + ' weeks. Let me connect you with someone who can help schedule your appointment.',
                action_required: 'transfer_to_agent',
                transfer_reason: 'no_availability_after_' + totalDaysSearched + '_day_search',
                CRITICAL: 'All sliding windows exhausted. Transfer to agent for manual scheduling assistance.'
            }
        }
    };
}
...
async function executeRequest() {
    ...
    const action = $action;
    console.log('[' + toolName + '] ' + TOOL_VERSION + ' - 30-DAY SLIDING WINDOW SEARCH');
    ...
    try {
        // v56: Use sliding window search for slots/grouped_slots
        if (action === 'slots' || action === 'grouped_slots') {
            const headers = { 'Content-Type': 'application/json' };
            const authHeader = getAuthHeader();
            if (authHeader) headers['Authorization'] = authHeader;

            const searchResult = await searchSlotsWithSlidingWindow(action, params, uui, headers);
